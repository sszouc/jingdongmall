...
(
    ...
    role              TINYINT     NOT NULL COMMENT '角色：0普通用户，1管理员，2超级管理员',
    created_time      DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_time      DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    status            TINYINT     NOT NULL COMMENT '状态：1正常，0禁用',
    banned_start_time DATETIME COMMENT '封禁开始时间',
    banned_end_time   DATETIME COMMENT '封禁结束时间',
    PRIMARY KEY (id),
    CHECK (role IN (0, 1, 2)),
    CHECK (status IN (1, 0))
) ENGINE = InnoDB
...
(
    id           INT AUTO_INCREMENT COMMENT '封禁ID'
        PRIMARY KEY,
    user_id      INT                               NOT NULL COMMENT '用户ID（一个用户只能有一条有效封禁）',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT = '用户封禁表';
...
-- 5.商品分类表（更新：theme_color字段名修正，level级别增加）
...
(
    ...
    theme_color  VARCHAR(100) NULL     DEFAULT NULL COMMENT '颜色',
    created_time DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_time DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id),
...
(
    ...
    -- 订单状态
    status               TINYINT         NOT NULL COMMENT '订单状态：0待付款，1待发货，2待收货，3已完成，4已取消，5退款中，6退款成功，7退款失败',
